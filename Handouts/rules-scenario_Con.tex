...
not represent the opinions of the GMs, The Luminary Roleplay Society, or the \studentgroupname.
...
Dungeons and Delegates, written by  Matt Fisher, Yvonne Kung, Paul Kivelson, Winfield J. Brown, Mahayah Phillips, will run on Saturday \gamedate, from 17:00 - 23:00 including time for rules explanation, character review, questions, play time and post game wrap up.

Half way through the game, the banquette will commence in the dining hall. Onyx has asked the Count Wilhelm Von Dorburt to run the affair. Immediately following a discussion of what to do with the armies is scheduled for any representatives as of yet undecided. 

Onyx's castle is in the middle of contested territory between the humans and monsters. Assuming it is functional, the teleport rune in the teleporter room will activate at the end of game and move all of the delegates to safety. Any character that is not in the teleporter room at game end will be overrun by the human army.
...
section for more details. This game has a few items of the type ``Soul''. The description on these items specifies that this special type of item \emph{cannot} be transferred except by specific mechanics. If you think you might have a mechanic that interacts with souls but are not sure, as a GM.
...
\textbf{In this game, unless a specific mechanic tells you to destroy a specific item, you may not destroy an item. All items are indestructible and unstashable unless you know otherwise.}
...
All weapons have  an item card. Weapon effects are on the card.  To use a weapon, you must have it in your hand and unobstructed.  Display it in an obvious manner.  You cannot hold more than one weapon in a hand.  You may only use one melee weapon at a time.
...
Each character has a set of abilities at their disposal. Abilities cannot be traded, and many have restrictions on how often or in what situations they can be used. You can only show the ``use'' side of abilities to another player, and even then only when you chose to activate the ability.

Some characters may be part of a group that possesses special ``pool abilities''. These are abilities you have access to as a result of your group affiliation. However, all pool abilities have only a finite number of uses, and these are shared across all members of the pool. You may gain access to new pool abilities as a consequence of your membership. Whenever you use a pool ability, you must consult with a GM who may inform you there are no ``charges'' left for that ability -- you will not know how many uses are left until they are expired.
...
\paragraph{Soul Items:} Some items are of the type ``Soul.'' They cannot be lost, stolen, traded, etc, and are not revealed by searches. Souls can only be interacted with via specific mechanics.

\paragraph{Rope:} Rope is {\bf not} freely available in this game. You must have an item card called ``rope'' to use it in a mechanic or to tie someone up. To tie someone up, they must be either willing or helpless (ie: knocked out, restrained, etc); this takes 1 minute and is interruptible.  Untying them takes a 10-count.  If you get tied up with rope, you become restrained.  If you are conscious and left alone, you can wriggle free in five minutes.
